    ...
    <div className="min-h-screen pb-20">
      ...
      {result && (
        <div ref={resultsRef} className="max-w-7xl mx-auto px-6 space-y-6">
          ...
          {/* Disruption Score */}
          {(() => {
            const { calcDisruption } = require("@/lib/utils");
            const disruption = calcDisruption(result);
            const label =
              disruption >= 70
                ? "High disruption — models strongly disagree"
                : disruption >= 40
                  ? "Moderate disruption — some divergence"
                  : "Low disruption — models mostly agree";
            const color =
              disruption >= 70
                ? "text-red-600"
                : disruption >= 40
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm dark:bg-slate-900 dark:border-slate-800">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="h-4 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mb-1.5">
                      <div
                        className={`h-full rounded-full transition-all duration-700 ${barColor}`}
                        style={{ width: `${disruption}%` }}
                      />
                    </div>
